...
function initGlueShelf(currentUsername, showMemory, onToggle) {
  ...
  folderIcon.style.cssText = `position:fixed; top:${savedFolderPos.top}px; left:${savedFolderPos.left}px; z-index:5000; cursor:pointer; display:${currentUsername ? "block" : "none"}; transition: transform 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275);`;
  folderIcon.innerHTML = `<div style="display:flex; flex-direction:column; align-items:center; pointer-events:none;"><img src="${ASSET_PATH}folder.png" style="width:70px; height:auto; filter:sepia(1) saturate(10) hue-rotate(245deg) brightness(0.9) drop-shadow(2px 4px 6px rgba(0,0,0,0.3));"><span style="font-family:serif; font-size:14px; color:white; text-shadow:1px 1px 3px black;">Glue</span></div>`;
  ...
  vistaWindow.style.cssText = `display:${isShelfExpanded ? "flex" : "none"}; visibility:${isShelfExpanded ? "visible" : "hidden"}; opacity:${isShelfExpanded ? 1 : 0}; transform:${isShelfExpanded ? "scale(1)" : "scale(0.9) translateY(10px)"}; position:fixed; top:${savedWindowPos.top}px; left:${savedWindowPos.left}px; width:320px; height:400px; z-index:5001; flex-direction:column; pointer-events:${isShelfExpanded ? "auto" : "none"};`;
  vistaWindow.innerHTML = `<div class="vista-title-bar" id="shelf-drag-handle" style="cursor:move; user-select:none;"><div class="vista-title">Glue Shelf</div><img src="${ASSET_PATH}aero_close.png" id="shelf-close-btn" style="height:22px; cursor:pointer;"></div><div class="vista-content-area" id="shelf-items-container" style="display:grid; grid-template-columns:repeat(2,1fr); gap:15px; padding:20px; overflow-y:auto; flex-grow:1;"></div>`;
